...
| `From Node ID`     | Source node of the segment                                                |
| `To Node ID`       | Target node of the segment                                                  |
| `IsEdge`           | 1 if this segment is an undirected edge; 0 otherwise                        |
...
| `Shape`            | LINESTRING geometry for QGIS visualization (comma-delimited)                      |
...
- Node IDs are integers; coordinates in the `Shape` field follow WKT (`LINESTRING`) format.
